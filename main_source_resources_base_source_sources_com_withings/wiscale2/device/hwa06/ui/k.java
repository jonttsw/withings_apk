package com.withings.wiscale2.device.hwa06.ui;

import com.withings.wiscale2.MainActivity;
import com.withings.wiscale2.device.hwa06.ui.Hwa06PostInstallActivity;
/* compiled from: Hwa06PostInstallActivity.kt */
/* loaded from: classes5.dex */
final class k extends kotlin.jvm.internal.w implements ym0.a<nm0.y> {

    /* renamed from: a  reason: collision with root package name */
    final /* synthetic */ Hwa06PostInstallActivity f54353a;

    /* JADX INFO: Access modifiers changed from: package-private */
    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public k(Hwa06PostInstallActivity hwa06PostInstallActivity) {
        super(0);
        this.f54353a = hwa06PostInstallActivity;
    }

    @Override // ym0.a
    public final nm0.y invoke() {
        Hwa06PostInstallActivity.a aVar = Hwa06PostInstallActivity.f54317l;
        Hwa06PostInstallActivity hwa06PostInstallActivity = this.f54353a;
        hwa06PostInstallActivity.getClass();
        MainActivity.f47950r.getClass();
        hwa06PostInstallActivity.startActivity(MainActivity.a.c());
        hwa06PostInstallActivity.finish();
        return nm0.y.f85009a;
    }
}
